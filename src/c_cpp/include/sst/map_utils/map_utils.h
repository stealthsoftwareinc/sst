...
inline Value FindWithDefault(
    const Key& key, const std::map<Key, Value>& input, const Value& default_value) {
...
template<typename Key, typename Value>
inline const Value* FindOrNull(const Key& key, const std::map<Key, Value>& input) {
  typename std::map<Key, Value>::const_iterator itr = input.find(key);
  if (itr == input.end()) return nullptr;
  return &(itr->second);
}
...
template<typename Key, typename Value>
inline const Value* FindOrNull(const Key& key, const std::map<Key, const Value>& input) {
  typename std::map<Key, Value>::const_iterator itr = input.find(key);
  if (itr == input.end()) return nullptr;
  return &(itr->second);
}
