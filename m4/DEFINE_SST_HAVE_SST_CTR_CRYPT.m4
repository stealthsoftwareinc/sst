...
AC_DEFUN([DEFINE_SST_HAVE_SST_CTR_CRYPT], [[{

#
# The block that contains this comment is the expansion of the
# DEFINE_SST_HAVE_SST_CTR_CRYPT macro.
#]dnl
m4_ifdef(
  [DEFINE_SST_HAVE_SST_CTR_CRYPT_HAS_BEEN_CALLED],
  [gatbps_fatal([
    DEFINE_SST_HAVE_SST_CTR_CRYPT has already been called
  ])],
  [m4_define([DEFINE_SST_HAVE_SST_CTR_CRYPT_HAS_BEEN_CALLED])])[]dnl
m4_if(
  m4_eval([$# != 0]),
  [1],
  [gatbps_fatal([
    DEFINE_SST_HAVE_SST_CTR_CRYPT requires exactly 0 arguments
    ($# ]m4_if([$#], [1], [[was]], [[were]])[ given)
  ])])[]dnl

AC_REQUIRE([DEFINE_HAVE_NETTLE_NETTLE_TYPES_H])
AC_REQUIRE([DEFINE_HAVE_UINT8_T_IN_NETTLE_NETTLE_TYPES_H])
AC_REQUIRE([DEFINE_SST_HAVE_SST_CTR_CRYPT_STEP])
