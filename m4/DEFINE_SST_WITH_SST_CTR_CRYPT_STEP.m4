...
AC_DEFUN_ONCE([DEFINE_SST_WITH_SST_CTR_CRYPT_STEP], [[{

#
# The block that contains this comment is the expansion of the
# DEFINE_SST_WITH_SST_CTR_CRYPT_STEP macro.
#]dnl
m4_ifdef(
  [DEFINE_SST_WITH_SST_CTR_CRYPT_STEP_HAS_BEEN_CALLED],
  [gatbps_fatal([
    DEFINE_SST_WITH_SST_CTR_CRYPT_STEP has already been called
  ])],
  [m4_define([DEFINE_SST_WITH_SST_CTR_CRYPT_STEP_HAS_BEEN_CALLED])])[]dnl
m4_if(
  m4_eval([$# != 0]),
  [1],
  [gatbps_fatal([
    DEFINE_SST_WITH_SST_CTR_CRYPT_STEP requires exactly 0 arguments
    ($# ]m4_if([$#], [1], [[was]], [[were]])[ given)
  ])])[]dnl

AC_REQUIRE([DEFINE_WITH_CTR_CRYPT])
