...
AC_DEFUN_ONCE([DEFINE_WITH_AES_BLOCK_SIZE], [[{

#
# The block that contains this comment is the expansion of the
# DEFINE_WITH_AES_BLOCK_SIZE macro.
#]dnl
m4_ifdef(
  [DEFINE_WITH_AES_BLOCK_SIZE_HAS_BEEN_CALLED],
  [gatbps_fatal([
    DEFINE_WITH_AES_BLOCK_SIZE has already been called
  ])],
  [m4_define([DEFINE_WITH_AES_BLOCK_SIZE_HAS_BEEN_CALLED])])[]dnl
m4_if(
  m4_eval([$# != 0]),
  [1],
  [gatbps_fatal([
    DEFINE_WITH_AES_BLOCK_SIZE requires exactly 0 arguments
...
GATBPS_CHECK_COMPILE(
  ...
  [
    unsigned char (* x1)[AES_BLOCK_SIZE] = 0;
    (void)x1;
  ],
  [1
    && WITH_NETTLE_AES_H
