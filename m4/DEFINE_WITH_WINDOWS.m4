...
AC_DEFUN_ONCE([DEFINE_WITH_WINDOWS], [
...
{ :
  ...
  GATBPS_CHECK_EXPR(
    [whether to use the Windows API],
    [WITH_WINDOWS],
    [(1
      ) && (0
        || ]GATBPS_SOFT_VAR([WITH_BUILD_GROUP_C_AUTOTOOLS])[
  ...
  [
    ...
    case $][{WYNA_WINDOWS_IS_YES?}$][{WITH_WINDOWS?} in 10)
      ]GATBPS_BARF([
        The Windows API is not available.
      ])[
    esac
