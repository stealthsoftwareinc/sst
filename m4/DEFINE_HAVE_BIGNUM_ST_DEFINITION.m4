...
AC_DEFUN_ONCE([DEFINE_HAVE_BIGNUM_ST_DEFINITION], [
...
{ :
  ...
  GATBPS_REQUIRE([DEFINE_WITH_OPENSSL_CRYPTO], [soft])
  ...
  m4_pushdef(
    [body],
    [[
      (void)sizeof(struct bignum_st);
    ]])

  AC_LANG_PUSH([C])

  GATBPS_CHECK_COMPILE(
    [whether bignum_st is fully defined],
    [HAVE_BIGNUM_ST_DEFINITION],
    prologue,
    body,
    [1
      && ]GATBPS_SOFT_VAR([WITH_BUILD_GROUP_C_AUTOTOOLS])[
      && ]GATBPS_SOFT_VAR([WITH_OPENSSL_CRYPTO])[
    ])
