...
AC_DEFUN_ONCE([DEFINE_WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT], [[{

#
# The block that contains this comment is the expansion of the
# DEFINE_WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT macro.
#]dnl
m4_ifdef(
  [DEFINE_WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT_HAS_BEEN_CALLED],
  [gatbps_fatal([
    DEFINE_WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT has already been called
  ])],
  [m4_define([DEFINE_WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT_HAS_BEEN_CALLED])])[]dnl
m4_if(
  m4_eval([$# != 0]),
  [1],
  [gatbps_fatal([
    DEFINE_WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT requires exactly 0
    arguments
...
GATBPS_CHECK_COMPILE(
  [nettle_crypt_func (unsigned int)],
  [WITH_NETTLE_CRYPT_FUNC_UNSIGNED_INT],
  [
